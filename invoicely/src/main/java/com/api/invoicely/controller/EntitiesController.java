...
public class EntitiesController {
    ...
    @PostMapping("/create")
    public ResponseEntity<EntitiesResponseDTO> createEntity(@AuthenticationPrincipal User owner, @RequestBody EntitiesCreateDTO dto) {
        return ResponseEntity.ok(entitiesService.createEntity(owner, dto));
    }

    @PutMapping("/update/{entityId}")
    public ResponseEntity<EntitiesResponseDTO> updateEntity(@AuthenticationPrincipal User owner, @PathVariable UUID entityId,
                                                            @RequestBody EntitiesUpdateDTO dto) {
        return ResponseEntity.ok(entitiesService.updateEntity(owner, entityId, dto));
    }

    @GetMapping("/{entityId}")
    public ResponseEntity<EntitiesResponseDTO> getEntityById(@AuthenticationPrincipal User owner, @PathVariable UUID entityId) {
        return ResponseEntity.ok(entitiesService.getEntityById(owner, entityId));
    }

    @GetMapping
    public ResponseEntity<List<EntitiesResponseDTO>> getAllEntities(@AuthenticationPrincipal User owner, @RequestParam(required = false) String type) {
    ...
    public ResponseEntity<Void> deleteEntity(@AuthenticationPrincipal User user, @PathVariable UUID entityId) {
